  ...
  return (
    <g className="fill-current font-mono text-[10px] text-muted-foreground">
      {ctx.data.map((row, i) => {
        ...
        const onRoundedStep = tickStep && Number.isFinite(numeric) && numeric % tickStep === 0
        if (tickStep ? !onRoundedStep && i !== ctx.dataLength - 1 : i % step !== 0 && i !== ctx.dataLength - 1) return null
