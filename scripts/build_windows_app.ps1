...
try {
    ...
    Run $Csc (@("/nologo", "/target:winexe", "/platform:x64", "/out:$Bundle/Modelfiche.exe", "/win32manifest:scripts/windows/app.manifest") + $Refs + @("scripts/windows/Launcher.cs"))
    Run $Csc (@("/nologo", "/target:exe", "/platform:x64", "/define:CLI", "/out:$Bundle/mfiche.exe", "/win32manifest:scripts/windows/app.manifest") + $Refs + @("scripts/windows/Launcher.cs"))
